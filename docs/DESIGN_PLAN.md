...
KB VRSTVY (statická data — co stavíme):
  ...
  TERRAIN_INSTANCE     — konkrétní lokalita s anchor evidence (Lake Flixton, Wash...)
  BIOTOPE              — definice ekologické komunity + CAN_HOST hrany na terrain
  ECOTONE              — přechod dvou biotopů, immutable
  SITE_INSTANCE        — archeologická lokalita jako epistemická kotva; váže se na TERRAIN_INSTANCE a BIOTOPE
...
Archeologická lokalita — epistemická kotva pro `tst_` a `bt_` záznamy. **Není simulační entita** — neslouží k modelování pohybu skupin, ale k vědecké validaci terrain a biotop záznamů.

Klíčové atributy:
- `terrain_instance` — reference na `tf_` (kde lokalita leží)
- `biotope` — reference na `bt_` (biotop v době osídlení)
- `lakescape_role` — `primary_camp | island_site | shore_camp | find_scatter`
- `location` — lat/lon + OS grid reference
- `site_area_m2` + `excavated_pct`
- `snapshot_reference_period` — období ze kterého pochází důkazy (nemusí být 6200 BCE)
- `snapshot_6200_bce.status` — stav lokality v době snapshotu (`active | post-occupation | submerged`)
- `assemblage_ref.summary` — velmi stručný popis nálezů
- `assemblage_ref.external` — odkaz na plný archiv (ADS, DOI)

**Temporální extrapolace (klíčové rozhodnutí):** Pokud `snapshot_reference_period` ≠ 6200 BCE, KB používá data jako DIRECT evidence pro terrain a biotop, ale `revision_note` dokumentuje časový posun a zdůvodnění srovnatelnosti podmínek. Snapshot KB zůstává 6200 BCE — nemáme druhý snapshot.

### 4.5 Budoucí node typy (vrstvy 4–9)
...
### 4.6 Epistemická vrstva (povinná u každého uzlu a hrany)
...
*Příklad revize: Star Carr jako celoroční sídliště [REFUTED, Clark 1954] → jarní/letní sídliště [VALID, DIRECT, Legge & Rowley-Conwy 1988; Milner et al. 2018]. Zimní osídlení (původní Clarkova interpretace) bylo odmítnuto reanalýzou fauny.*
...
- **OS Terrain 50** — digitální model terénu (5 m nebo 50 m rozlišení)
- **BGS Geology** — geologické podklady pro substrate atributy
...
| Archeologické lokality | bod | SITE_INSTANCE záznamy — Lake Flixton lakescape |
...
- **SQLite + SpatiaLite** — persistentní úložiště s prostorovými dotazy
- **NetworkX** — runtime grafová reprezentace pro relační dotazy
...
- Clark, J.G.D. (1954): Excavations at Star Carr. Cambridge UP. **[primární excavace]**
- Legge, A.J. & Rowley-Conwy, P.A. (1988): Star Carr Revisited. Centre for Extra-Mural Studies. **[sezónnost — SPRING/SUMMER konsensus]**
- Mellars, P. & Dark, P. (1998): Star Carr in Context. McDonald Institute. **[dřevěná platforma]**
...
- Palmer, A.P. et al. (2015): Evolution of Palaeolake Flixton — LGIT lacustrine sequences. Proceedings of the Geologists' Association 126(1). **[bathymetrie, hladina jezera]**
- Taylor, B. & Alison, R. (2018): Lake level reconstruction — Lake Flixton ~24m aOD. In: Milner et al. 2018. **[shoreline kontůra]**
...
- University of York (2018): Star Carr and Lake Flixton archives. ADS. https://doi.org/10.5284/1041580 **[GIS data lakescape]**
...
- **Domestikace psa:** doložena na Star Carr (~9000 BCE); vliv na lovecké strategie
- **Dopad Storegga tsunami:** demografický dopad na mezolitické komunity
- **ADS GIS stažení:** registrace na archaeologydataservice.ac.uk — stáhnout GIS sekci archivu doi:10.5284/1041580 pro geometrii Lake Flixton a souřadnice 24+ lokalit
- **SITE_INSTANCE schéma:** finalizovat v schema_examples v05 — zatím jen v design plánu
